...
class PV(IGES):
    ...
    def __init__(self, num_h, mdl, pv_set_max, set_price, ha0, eff, set_name='PV'):
        ...
        self.p_pv = mdl.continuous_var_list([i for i in range(0, num_h)], name='p_pv{0}'.format(PV.index))
...
class LiBrRefrigeration(IGES):
    ...
    def __init__(self, num_h, mdl, xhl_set_max, set_price, eff, set_name='LiBrRefrigeration'):
        IGES(set_name)
        LiBrRefrigeration.index += 1
        self.num_h = num_h
        self.xhl_set = mdl.continuous_var(name='xhl_set{0}'.format(LiBrRefrigeration.index))

        self.h_xhl_from = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='h_xhl_from{0}'.format(LiBrRefrigeration.index))
        self.c_xhl = mdl.continuous_var_list([i for i in range(0, self.num_h)],
...
class Diesel(IGES):
    ...
    def __init__(self, num_h, mdl, diesel_set_max, set_price, run_price, set_name='diesel'):
        IGES(set_name)
        Diesel.index += 1
        self.num_h = num_h
        self.diesel_set = mdl.continuous_var(name='diesel_set{0}'.format(Diesel.index))
        self.p_diesel = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                name='p_diesel{0}'.format(Diesel.index))
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(self.p_diesel[i] <= self.diesel_set for i in range(0, self.num_h))
        mdl.add_constraint(
            self.nianhua == self.diesel_set * self.set_price / 15 + self.p_sum * self.run_price * 8760 / self.num_h)
...
class ESS(IGES):
    ...
    def __init__(self, num_h, mdl, ess_set_max, ess_price, pcs_price, c_rate_max, eff, ess_init, soc_min, soc_max,
                 set_name='ess'):
        IGES(set_name)
        ESS.index += 1

        self.ess_set = mdl.continuous_var(name='ess_set{0}'.format(ESS.index))
        self.p_ess = mdl.continuous_var_list([i for i in range(0, num_h)], lb=-bigM,
                                             name='p_ess{0}'.format(ESS.index))
        # 充电功率
        self.p_ess_ch = mdl.continuous_var_list([i for i in range(0, num_h)], name='p_ess_ch{0}'.format(ESS.index))
        # 放电功率
        self.p_ess_dis = mdl.continuous_var_list([i for i in range(0, num_h)], name='p_ess_dis{0}'.format(ESS.index))
        # 能量
        self.ess = mdl.continuous_var_list([i for i in range(0, num_h)], name='ess{0}'.format(ESS.index))
        ...
        self.ch_flag = mdl.binary_var_list([i for i in range(0, num_h)],
                                           name='bess_ch_flag{0}'.format(ESS.index))  # 充电
        self.dis_flag = mdl.binary_var_list([i for i in range(0, num_h)],
                                            name='bess_dis_flag{0}'.format(ESS.index))  # 放电
        # 效率
        self.eff = eff
    ...
    def cons_register(self, mdl, regester_period_constraints=1, day_node=24):
        bigM = 1e10
        ...
        for day in range(1, int(self.num_h / day_node) + 1):
            mdl.add_constraints(self.ess[i] == self.ess[i - 1] + (
                    self.p_ess_ch[i] * self.eff - self.p_ess_dis[i] / self.eff) * simulationT / 3600 for i in
                                range(1 + day_node * (day - 1), day_node * day))
        ...
        if regester_period_constraints == 1:
            ...
            # 两天之间的连接
            mdl.add_constraints(
                self.ess[i] == self.ess[i - 1] + (
                        self.p_ess_ch[i] * self.eff - self.p_ess_dis[i] / self.eff) * simulationT / 3600
...
class ESSVariable(IGES):
    ...
    def __init__(self, num_h, mdl, ess_set_max, ess_price, pcs_price, c_rate_max, eff, ess_init, soc_min, soc_max,
                 set_name='ess_variable'):
        IGES(set_name)
        ESSVariable.index += 1

        self.ess_set = mdl.continuous_var_list([i for i in range(0, num_h)],
                                               name='essVariable_set{0}'.format(ESSVariable.index))
        self.p_ess = mdl.continuous_var_list([i for i in range(0, num_h)], lb=-bigM,
                                             name='p_essVariable{0}'.format(ESSVariable.index))
        # 充电功率
        self.p_ess_ch = mdl.continuous_var_list([i for i in range(0, num_h)],
                                                name='p_essVariable_ch{0}'.format(ESSVariable.index))
        # 放电功率
        self.p_ess_dis = mdl.continuous_var_list([i for i in range(0, num_h)],
                                                 name='p_essVariable_dis{0}'.format(ESSVariable.index))
        # 能量
        self.ess = mdl.continuous_var_list([i for i in range(0, num_h)],
                                           name='essVariable{0}'.format(ESSVariable.index))
        self.ess_set_max = ess_set_max
        self.ess_price = ess_price
        self.pcs_price = pcs_price
        self.num_h = num_h
        self.pcs_set = mdl.continuous_var_list([i for i in range(0, num_h)],
                                               name='pcs_setVariable{0}'.format(ESSVariable.index))  # pcs
        self.ch_flag = mdl.binary_var_list([i for i in range(0, num_h)],
                                           name='bessVariable_ch_flag{0}'.format(ESSVariable.index))  # 充电
        self.dis_flag = mdl.binary_var_list([i for i in range(0, num_h)],
                                            name='bessVariable_dis_flag{0}'.format(ESSVariable.index))  # 放电
        # 效率
        self.eff = eff
        self.c_rate_max = c_rate_max
        self.ess_init = ess_init
...
class Csgr(IGES):
    ...
    def __init__(self, num_h, mdl, csgr_set_max, csgr_price, csgrgtxr_price, ha0, eff, set_name='csgr'):
        IGES(set_name)
        Csgr.index += 1
        self.num_h = num_h
        self.csgr_set = mdl.continuous_var(name='csgr_set{0}'.format(Csgr.index))
        self.p_csgr = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                              name='p_csgr{0}'.format(Csgr.index))
        self.p_csgr_steam = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='p_csgr_steam{0}'.format(Csgr.index))
        ...
        self.csgrgtxr_set = ESS(num_h, mdl, self.csgrgtxr_set_max, self.csgrgtxr_price, pcs_price=100, c_rate_max=2,
                                eff=0.9, ess_init=1, soc_min=0, soc_max=1)
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.p_csgr[h] + self.csgrgtxr_set.p_ess[h] == self.p_csgr_steam[h] for h in hrange)  # 槽式光热系统产生的高温
        mdl.add_constraints(0 <= self.p_csgr_steam[h] for h in hrange)  # 约束能量不能倒流
        mdl.add_constraint(self.nianhua == self.csgr_set * self.csgr_price / 15 + self.csgrgtxr_set.nianhua)
...
class CHP(IGES):
    ...
    def __init__(self, num_h, mdl, chp_num_max, chp_price, gas_price, chp_single_set, drratio, set_name='chp'):
        IGES(set_name)
        CHP.index += 1
        self.num_h = num_h
        self.chp_set = mdl.continuous_var(name='chp_set{0}'.format(CHP.index))
        self.p_chp = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                             name='p_chp{0}'.format(CHP.index))
        self.h_chp = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                             name='h_chp{0}'.format(CHP.index))
        self.gas_chp = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                               name='gas_chp{0}'.format(CHP.index))  # 时时耗气量
        self.chp_price = chp_price
        self.gas_price = gas_price
        self.chp_open_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='chp_open_flag{0}'.format(CHP.index))
        ...
        # 机组数量
        self.chp_run_num = mdl.integer_var_list([i for i in range(0, self.num_h)],
                                                name='chp_run_num{0}'.format(CHP.index))
        ...
        self.gts_set = Exchanger(self.num_h, mdl, self.chp_set * 0.5, set_price=300, k=0)
        self.yqyrwater_set = Exchanger(self.num_h, mdl, self.chp_set * 0.5, set_price=300, k=0)
        self.yqyrsteam_set = Exchanger(self.num_h, mdl, self.chp_set * 0.5, set_price=300, k=0)
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.chp_open_flag[h] * self.chp_single_set * self.chp_limit_down_ratio <= self.p_chp[h] for h in hrange)
        ...
        mdl.add_constraints(
            self.chp_run_num[h] * self.chp_single_set <= self.p_chp[h] + self.chp_single_set + 1 for h in
            hrange)  # 确定CHP开启台数
        ...
        mdl.add_constraint(
            self.nianhua == self.chp_num * self.chp_single_set * self.chp_price / 15 + self.gts_set.nianhua + self.yqyrwater_set.nianhua + self.yqyrsteam_set.nianhua + self.gas_cost * 8760 / self.num_h)
...
class Gasgl(IGES):
    ...
    def __init__(self, num_h, mdl, gasgl_set_max, gasgl_price, gas_price, eff, set_name='gasgl'):
        IGES(set_name)
        Gasgl.index += 1
        self.num_h = num_h
        self.gasgl_set = mdl.continuous_var(name='gasgl_set{0}'.format(Gasgl.index))

        self.h_gasgl = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                               name='h_gasgl{0}'.format(Gasgl.index))
        self.gas_gasgl = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='gas_gasgl{0}'.format(Gasgl.index))  # 时时耗气量
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraint(
            self.nianhua == self.gasgl_set * self.gasgl_price / 15 + self.gas_cost * 8760 / self.num_h)
...
class Dgl(IGES):
    ...
    def __init__(self, num_h, mdl, dgl_set_max, dgl_price, ele_price, eff, set_name='dgl'):
        IGES(set_name)
        Dgl.index += 1
        self.num_h = num_h
        self.dgl_set = mdl.continuous_var(name='dgl_set{0}'.format(Dgl.index))
        self.h_dgl = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                             name='h_dgl{0}'.format(Dgl.index))
        self.ele_dgl = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                               name='ele_dgl{0}'.format(Dgl.index))  # 时时耗气量
...
class Exchanger(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, k, set_name='exchanger'):
        IGES(set_name)
        # k 传热系数
        Exchanger.index += 1
        self.num_h = num_h
        ...
        self.h_exch = mdl.continuous_var_list([i for i in range(0, self.num_h)],
...
class AirHeatPump(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, set_name='air_heat_pump'):
        IGES(set_name)
        self.num_h = num_h
        AirHeatPump.index += 1
        self.ele_price = ele_price
        self.rb_set = mdl.continuous_var(name='rb_set{0}'.format(AirHeatPump.index))
        self.nianhua = mdl.continuous_var(name='AirHeatPump_nianhua{0}'.format(AirHeatPump.index))
        self.ele_cost = mdl.continuous_var(name='AirHeatPump_ele_cost{0}'.format(AirHeatPump.index))
        self.set_price = set_price
        self.set_max = set_max
        self.p_rb_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_rb_cool{0}'.format(AirHeatPump.index))
        self.cool_rb_out = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='cool_rb_out{0}'.format(AirHeatPump.index))

        self.rb_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='rb_cool_flag{0}'.format(AirHeatPump.index))

        self.p_rb_xcool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_rb_xcool{0}'.format(AirHeatPump.index))
        self.xcool_rb_out = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='xcool_rb_out{0}'.format(AirHeatPump.index))

        self.rb_xcool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='rb_xcool_flag{0}'.format(AirHeatPump.index))
        self.p_rb_heat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_rb_heat{0}'.format(AirHeatPump.index))
        self.heat_rb_out = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='heat_rb_out{0}'.format(AirHeatPump.index))
        self.rb_heat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='rb_heat_flag{0}'.format(AirHeatPump.index))
        self.p_rb_xheat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_rb_xheat{0}'.format(AirHeatPump.index))
        self.xheat_rb_out = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='xheat_rb_out{0}'.format(AirHeatPump.index))
        self.rb_xheat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='rb_xheat_flag{0}'.format(AirHeatPump.index))
        self.ele_rb = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                              name='ele_rb{0}'.format(AirHeatPump.index))
        self.p_rb = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                            name='p_rb{0}'.format(AirHeatPump.index))
        self.cop_rb_cool = 3
        self.cop_rb_xcool = 3
        self.cop_rb_heat = 3
        self.cop_rb_xheat = 3
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(self.p_rb_cool[h] <= self.cool_rb_out[h] * self.rb_set / 100 for h in hrange)
        mdl.add_constraints(self.p_rb_cool[h] <= bigM * self.rb_cool_flag[h] for h in hrange)

        mdl.add_constraints(0 <= self.p_rb_xcool[h] for h in hrange)
        mdl.add_constraints(self.p_rb_xcool[h] <= self.xcool_rb_out[h] * self.rb_set / 100 for h in hrange)
        mdl.add_constraints(self.p_rb_xcool[h] <= bigM * self.rb_xcool_flag[h] for h in hrange)

        mdl.add_constraints(0 <= self.p_rb_heat[h] for h in hrange)
        mdl.add_constraints(self.p_rb_heat[h] <= self.heat_rb_out[h] * self.rb_set / 100 for h in hrange)
        mdl.add_constraints(self.p_rb_heat[h] <= bigM * self.rb_heat_flag[h] for h in hrange)

        mdl.add_constraints(0 <= self.p_rb_xheat[h] for h in hrange)
        mdl.add_constraints(self.p_rb_xheat[h] <= self.xheat_rb_out[h] * self.rb_set / 100 for h in hrange)
        mdl.add_constraints(self.p_rb_xheat[h] <= bigM * self.rb_xheat_flag[h] for h in hrange)

        mdl.add_constraints(
            self.rb_cool_flag[h] + self.rb_xcool_flag[h] + self.rb_heat_flag[h] + self.rb_xheat_flag[h] == 1 for h in
            hrange)
        mdl.add_constraints(
            self.ele_rb[h] == self.p_rb_cool[h] / self.cop_rb_cool[h] + self.p_rb_xcool[h] / self.cop_rb_xcool[h] + \
            self.p_rb_heat[h] / self.cop_rb_heat[h] + self.p_rb_xheat[h] / self.cop_rb_xheat[h] for h in hrange)
        mdl.add_constraints(
            self.p_rb[h] == self.p_rb_cool[h] + self.p_rb_xcool[h] + self.p_rb_heat[h] + self.p_rb_xheat[h] for h in
            hrange)

        self.ele_cost = mdl.sum(self.ele_rb[h] * self.ele_price[h] for h in hrange)
        # 年化
        mdl.add_constraint(self.nianhua == self.rb_set * self.set_price / 15 + self.ele_cost * 8760 / self.num_h)
...
class WaterHeatPump(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, case_ratio, set_name='water_heat_pump'):
        IGES(set_name)
        ...
        self.p_sy_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_sy_cool{0}'.format(WaterHeatPump.index))

        self.sy_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='sy_cool_flag{0}'.format(WaterHeatPump.index))

        self.p_sy_xcool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_sy_xcool{0}'.format(WaterHeatPump.index))

        self.sy_xcool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='sy_xcool_flag{0}'.format(WaterHeatPump.index))
        self.p_sy_heat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_sy_heat{0}'.format(WaterHeatPump.index))

        self.sy_heat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='sy_heat_flag{0}'.format(WaterHeatPump.index))
        self.p_sy_xheat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_sy_xheat{0}'.format(WaterHeatPump.index))

        self.sy_xheat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='sy_xheat_flag{0}'.format(WaterHeatPump.index))
        self.ele_sy = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                              name='ele_sy{0}'.format(WaterHeatPump.index))
        self.p_sy = mdl.continuous_var_list([i for i in range(0, self.num_h)],
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.sy_cool_flag[h] + self.sy_xcool_flag[h] + self.sy_heat_flag[h] + self.sy_xheat_flag[h] == 1 for h in
            hrange)
        mdl.add_constraints(
            self.ele_sy[h] == self.p_sy_cool[h] / self.cop_sy_cool + self.p_sy_xcool[h] / self.cop_sy_xcool + \
            self.p_sy_heat[h] / self.cop_sy_heat + self.p_sy_xheat[h] / self.cop_sy_xheat for h in hrange)
        mdl.add_constraints(
            self.p_sy[h] == self.p_sy_cool[h] + self.p_sy_xcool[h] + self.p_sy_heat[h] + self.p_sy_xheat[h] for h in
            hrange)
...
class WaterCooledScrew(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, case_ratio, set_name='water_cooled_screw'):
        IGES(set_name)
        self.num_h = num_h
        ...
        self.set_max = set_max
        self.case_ratio = case_ratio
        self.p_slj_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_slj_cool{0}'.format(WaterCooledScrew.index))

        self.slj_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='slj_cool_flag{0}'.format(WaterCooledScrew.index))

        self.p_slj_xcool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='p_slj_xcool{0}'.format(WaterCooledScrew.index))

        self.slj_xcool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                  name='slj_xcool_flag{0}'.format(WaterCooledScrew.index))

        self.ele_slj = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                               name='ele_slj{0}'.format(WaterCooledScrew.index))
        self.p_slj = mdl.continuous_var_list([i for i in range(0, self.num_h)],
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.ele_slj[h] == self.p_slj_cool[h] / self.cop_slj_cool + self.p_slj_xcool[h] / self.cop_slj_xcool for h
            in hrange)
...
class DoubleGK(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, case_ratio, set_name='doublegk'):
        IGES(set_name)
        self.num_h = num_h
        DoubleGK.index += 1
        self.ele_price = ele_price
        ...
        self.set_max = set_max
        self.case_ratio = case_ratio
        self.p_doublegk_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                       name='p_doublegk_cool{0}'.format(DoubleGK.index))

        self.doublegk_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                      name='doublegk_cool_flag{0}'.format(DoubleGK.index))

        self.p_doublegk_ice = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                      name='p_doublegk_ice{0}'.format(DoubleGK.index))

        self.doublegk_ice_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                     name='doublegk_ice_flag{0}'.format(DoubleGK.index))

        self.ele_doublegk = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='ele_doublegk{0}'.format(DoubleGK.index))
        self.p_doublegk = mdl.continuous_var_list([i for i in range(0, self.num_h)],
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.ele_doublegk[h] == self.p_doublegk_cool[h] / self.cop_doublegk_cool + self.p_doublegk_ice[
                h] / self.cop_doublegk_ice for h in hrange)
...
class ThreeGK(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, case_ratio, set_name='threegk'):
        IGES(set_name)
        self.num_h = num_h
        ...
        self.set_max = set_max
        self.case_ratio = case_ratio
        self.p_threegk_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                      name='p_threegk_cool{0}'.format(ThreeGK.index))

        self.threegk_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                     name='threegk_cool_flag{0}'.format(ThreeGK.index))

        self.p_threegk_ice = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                     name='p_threegk_ice{0}'.format(ThreeGK.index))

        self.threegk_ice_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                    name='threegk_ice_flag{0}'.format(ThreeGK.index))

        self.p_threegk_heat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                      name='p_threegk_heat{0}'.format(ThreeGK.index))

        self.threegk_heat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                     name='threegk_heat_flag{0}'.format(ThreeGK.index))

        self.ele_threegk = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='ele_threegk{0}'.format(ThreeGK.index))
        self.p_threegk = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_threegk{0}'.format(ThreeGK.index))
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.threegk_cool_flag[h] + self.threegk_ice_flag[h] + self.threegk_heat_flag[h] == 1 for h in hrange)
        mdl.add_constraints(self.ele_threegk[h] == self.p_threegk_cool[h] / self.cop_threegk_cool + self.p_threegk_ice[
            h] / self.cop_threegk_ice + self.p_threegk_heat[h] / self.cop_threegk_heat for h in hrange)
        mdl.add_constraints(
            self.p_threegk[h] == self.p_threegk_cool[h] + self.p_threegk_ice[h] + self.p_threegk_heat[h] for h in
            hrange)
...
class GeothermalHeatPump(IGES):
    ...
    def __init__(self, num_h, mdl, set_max, set_price, ele_price, set_name='geothermal_heat_pump'):
        IGES(set_name)
        self.num_h = num_h
        ...
        self.ele_dire = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                name='ele_dire{0}'.format(GeothermalHeatPump.index))
        self.p_dire = mdl.continuous_var_list([i for i in range(0, self.num_h)],
...
class WaterEnergyStorage(IGES):
    # index=0
    def __init__(self, num_h, mdl, sx_V_max, v_price, pcs_price, c_rate_max, eff, ess_init, soc_min, soc_max,
                 ratio_cool, ratio_heat, ratio_gheat, set_name='water_energy_storage'):
        IGES(set_name)
        self.num_h = num_h
        self.mdl = mdl
        # 对于水蓄能，优化的变量为水罐的体积
        self.sx = ESSVariable(num_h, mdl, bigM, 0, pcs_price, c_rate_max, eff, ess_init, soc_min, soc_max)
        self.index = ESSVariable.index
        self.sx_set_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='sx_set_cool{0}'.format(self.index))
        self.sx_set_heat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                   name='sx_set_heat{0}'.format(self.index))
        self.sx_set_gheat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='sx_set_gheat{0}'.format(self.index))
        self.v_price = v_price
        self.sx_V_max = sx_V_max
        self.sx_V = mdl.continuous_var(name='sx_V{0}'.format(self.index))
        self.sx_cool_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='sx_cool_flag{0}'.format(self.index))
        self.sx_heat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                name='sx_heat_flag{0}'.format(self.index))
        self.sx_gheat_flag = mdl.binary_var_list([i for i in range(0, self.num_h)],
                                                 name='sx_gheat_flag{0}'.format(self.index))
        self.ratio_cool = ratio_cool
        self.ratio_heat = ratio_heat
        self.ratio_gheat = ratio_gheat
        self.p_sx_cool = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_sx_cool{0}'.format(self.index))
        self.p_sx_heat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_sx_heat{0}'.format(self.index))
        self.p_sx_gheat = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                  name='p_sx_gheat{0}'.format(self.index))
        self.nianhua = mdl.continuous_var(
            name='p_sx_nianhua{0}'.format(self.index))

    def cons_regester(self, mdl, regester_period_constraints, day_node):
        bigM = 1e10
        hrange = range(0, self.num_h)
        ...
        mdl.add_constraints(
            self.sx.ess_set[h] == self.sx_set_cool[h] + self.sx_set_heat[h] + self.sx_set_gheat[h] for h in hrange)
        # (1)
        ...
        mdl.add_constraints(
            self.sx_set_cool[h] >= self.sx_V * self.ratio_cool - (1 - self.sx_cool_flag[h]) * bigM for h in hrange)
        # (2)
        ...
        mdl.add_constraints(
            self.sx_set_heat[h] >= self.sx_V * self.ratio_heat - (1 - self.sx_heat_flag[h]) * bigM for h in hrange)
        # (3)
        ...
        mdl.add_constraints(
            self.sx_set_gheat[h] >= self.sx_V * self.ratio_gheat - (1 - self.sx_gheat_flag[h]) * bigM for h in hrange)
        # % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        mdl.add_constraints(
            self.sx_cool_flag[h] + self.sx_heat_flag[h] + self.sx_gheat_flag[h] == 1 for h in hrange)  # % 三个方面进行核算。
...
class Dyzqfsq(IGES):
    ...
    def __init__(self, num_h, mdl, dyzqfsq_set_max, dyzqfsq_price, dyzqfsqgtxr_price, ele_price, eff,
                 set_name='dyzqfsq'):
        IGES(set_name)
        Dyzqfsq.index += 1
        self.num_h = num_h
        self.dyzqfsq_set = mdl.continuous_var(name='dyzqfsq_set{0}'.format(Dyzqfsq.index))
        self.p_dyzqfsq = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                 name='p_dyzqfsq{0}'.format(Dyzqfsq.index))

        self.p_dyzqfsq_steam = mdl.continuous_var_list([i for i in range(0, self.num_h)],
        ...
        self.dyzqfsqgtxr_set = ESS(num_h, mdl, self.dyzqfsqgtxr_set_max, self.dyzqfsqgtxr_price, pcs_price=0,
                                   c_rate_max=2, eff=0.9, ess_init=1, soc_min=0, soc_max=1)
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraints(
            self.p_dyzqfsq[h] + self.dyzqfsqgtxr_set.p_ess[h] == self.p_dyzqfsq_steam[h] for h in hrange)  # 槽式光热系统产生的高温
        ...
        mdl.add_constraint(
            self.nianhua == self.dyzqfsq_set * self.dyzqfsq_price / 15 + self.dyzqfsqgtxr_set.nianhua + self.ele_cost)
...
class Linear_abs(object):
    ...
    def __init__(self, mdl, x, irange):
        Linearization.index += 1  # 要增加变量
        self.b_posi = mdl.binary_var_list([i for i in irange], name='b_posi_abs{0}'.format(Linear_abs.index))
        self.b_neg = mdl.binary_var_list([i for i in irange], name='b_neg_abs{0}'.format(Linear_abs.index))
        self.x_posi = mdl.continuous_var_list([i for i in irange], name='x_posi_abs{0}'.format(Linear_abs.index))
        self.x_neg = mdl.continuous_var_list([i for i in irange], name='x_neg_abs{0}'.format(Linear_abs.index))
        self.abs_x = mdl.continuous_var_list([i for i in irange], name='abs_x{0}'.format(Linear_abs.index))
        self.irange = irange
        self.x = x
    ...
    def abs_add_constraints(self, mdl):
        ...
        mdl.add_constraints(self.x_posi[i] <= self.bigM0 * self.b_posi[i] for i in self.irange)
        mdl.add_constraints(self.x_neg[i] >= 0 for i in self.irange)
        mdl.add_constraints(self.x_neg[i] <= self.bigM0 * self.b_neg[i] for i in self.irange)
        mdl.add_constraints(self.x[i] == self.x_posi[i] - self.x_neg[i] for i in self.irange)
        mdl.add_constraints(self.abs_x[i] == self.x_posi[i] + self.x_neg[i] for i in self.irange)
...
class CitySupply(IGES):
    ...
    def __init__(self, num_h, mdl, citysupply_set_max, set_price, run_price, eff, set_name='city_supply'):
        IGES(set_name)
        CitySupply.index += 1
        self.num_h = num_h
        self.citysupply_set = mdl.continuous_var(name='citysupply_set{0}'.format(CitySupply.index))

        self.h_citysupply = mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                    name='h_citysupply{0}'.format(CitySupply.index))
        self.h_citysupply_from = mdl.continuous_var_list([i for i in range(0, self.num_h)],
    ...
    def cons_register(self, mdl):
        ...
        mdl.add_constraint(
            self.nianhua == self.citysupply_set * self.set_price / 15 + self.citysupply_cost * 8760 / self.num_h)


class GridNet(IGES):
    index = 0

    def __init__(self, num_h, mdl, gridnet_set_max, set_price, ele_price_from, ele_price_to, set_name='grid_net'):
        IGES(set_name)
        GridNet.index += 1
        self.num_h = num_h
        self.mdl = mdl
        ...
        self.total_power = mdl1.continuous_var_list([i for i in range(0, num_h0)], lb=-bigM,
                                                    name='total_power {0}'.format(GridNet.index))
        self.powerfrom = mdl1.continuous_var_list([i for i in range(0, num_h0)],
                                                  name='powerfrom{0}'.format(GridNet.index))
        self.powerto = mdl1.continuous_var_list([i for i in range(0, num_h0)], name='powerto {0}'.format(GridNet.index))
        ...
        self.powerfrom_max = mdl1.continuous_var(name='powerfrom_max{0}'.format(GridNet.index))
        self.powerto_max = mdl1.continuous_var(name='powerto_max{0}'.format(GridNet.index))
    ...
    def cons_register(self, mdl, powerpeak_pre=2000):
        ...
        self.basecost = mdl1.min(mdl.max([self.powerpeak, powerpeak_pre]) * 31, self.gridnet_set * 22) * 12

        self.gridnet_cost = mdl.sum(
            self.powerfrom[h] * self.ele_price_from[h] + self.powerto[h] * self.ele_price_to for h in
            hrange) + self.basecost
        mdl.add_constraint(
            self.nianhua == self.gridnet_set * self.set_price / 15 + self.gridnet_cost * 8760 / self.num_h)
...
class Linearization(object):
    ...
    def max_zeros(self, num_h, mdl, x, y):
        ...
        y_flag = mdl1.binary_var_list([i for i in range(0, num_h)], name='y_flag{0}'.format(Linearization.index))
    ...
    def add(self, num_h, mdl, x1, x2):
        Linearization.index += 1
        add_y = mdl1.continuous_var_list([i for i in range(0, num_h)], name='add_y{0}'.format(Linearization.index))
    ...
    def posi_neg_cons_regester(self, num_h, mdl, x, xposi, xneg):
        Linearization.index += 1
        bigM = 1e10
        posi_flag = mdl1.binary_var_list([i for i in range(0, num_h)],
...
if __name__ == "__main__":
    ...
    bess = ESS(num_h0, mdl1, ess_set_max=20000, ess_price=1800, pcs_price=250, c_rate_max=2, eff=0.9, ess_init=1,
               soc_min=0, soc_max=1)
    bess.cons_register(mdl1, 1, day_node)
    # 高温蒸汽
    csgr = Csgr(num_h0, mdl1, 5000, 2000, 1000, ha0, 0.8)
    csgr.cons_register(mdl1)
    dyzqfsq = Dyzqfsq(num_h0, mdl1, dyzqfsq_set_max=20000, dyzqfsq_price=200, dyzqfsqgtxr_price=200,
                      ele_price=ele_price0, eff=0.9)
    dyzqfsq.cons_register(mdl1)
    chp = CHP(num_h0, mdl1, chp_num_max=5, chp_price=2000, gas_price=gas_price0, chp_single_set=2000, drratio=1.2)
    chp.cons_register(mdl1)
    gasgl = Gasgl(num_h0, mdl1, gasgl_set_max=5000, gasgl_price=200, gas_price=gas_price0, eff=0.9)
    gasgl.cons_register(mdl1)
    shizheng_steam = CitySupply(num_h0, mdl1, citysupply_set_max=5000, set_price=3000, run_price=0.3 * np.ones(num_h0),
                                eff=0.9)
    shizheng_steam.cons_register(mdl1)
    # 以上为蒸汽发生装置
    p_steam_used_product = mdl1.continuous_var_list([i for i in range(0, num_h0)],
                                                    name='p_steam_used_product')
    p_steam_used_heatcool = mdl1.continuous_var_list([i for i in range(0, num_h0)],
                                                     name='p_steam_used_heatcool')
    p_steam_sum = mdl1.continuous_var_list([i for i in range(0, num_h0)], name='p_steam_sum ')
    mdl1.add_constraints(
        p_steam_sum[h] == shizheng_steam.h_citysupply[h] + chp.yqyrsteam_set.h_exch[h] + csgr.p_csgr_steam[h] +
        dyzqfsq.p_dyzqfsq_steam[h] + gasgl.h_gasgl[h] for h in range(0, num_h0))
    # 高温蒸汽去处
    mdl1.add_constraints(p_steam_sum[h] >= steam_load[h] + p_steam_used_heatcool[h] for h in range(0, num_h0))
    ...
    mdl1.add_constraints(
        p_steam_used_heatcool[h] >= qs_exchanger.h_exch[h] + zq_xhl.h_xhl_from[h] for h in range(0, num_h0))
    # 高温热水
    # 1) chp gts
    # 2) chp yqyr_to_water
    # 3
    ...
    # 4
    xbxr = ESS(num_h0, mdl1, ess_set_max=10000, ess_price=350, pcs_price=0, c_rate_max=0.5, eff=0.9, ess_init=1,
               soc_min=0, soc_max=1)
    xbxr.cons_register(mdl1)
    # 5
    szrs = CitySupply(num_h0, mdl1, citysupply_set_max=10000, set_price=3000, run_price=cityrs_price0, eff=0.9)
    szrs.cons_register(mdl1)
    # 6
    rsdgl = Dgl(num_h0, mdl1, dgl_set_max=10000, dgl_price=200, ele_price=ele_price0, eff=0.9)
    rsdgl.cons_register(mdl1)
    # 7
    gasgl_rs = Gasgl(num_h0, mdl1, gasgl_set_max=20000, gasgl_price=200, gas_price=gas_price0, eff=0.9)
    gasgl_rs.cons_register(mdl1)
    sx = WaterEnergyStorage(num_h0, mdl1, sx_V_max=10000, v_price=300, pcs_price=1, c_rate_max=0.5, eff=0.9,
                            ess_init=1, soc_min=0, soc_max=1, ratio_cool=10, ratio_heat=10, ratio_gheat=20)

    sx.cons_regester(mdl1, 1, day_node)
    # 高温热水合计
    p_gws_sum = mdl1.continuous_var_list([i for i in range(0, num_h0)], name='p_gws_sum ')
    mdl1.add_constraints(
        p_gws_sum[h] == chp.gts_set.h_exch[h] + chp.yqyrwater_set.h_exch[h] + pbgr.p_pv[h] + xbxr.p_ess[h] +
        szrs.h_citysupply[h] + gasgl_rs.h_gasgl[h] + rsdgl.h_dgl[h] + sx.p_sx_gheat[h]
    ...
    # 高温热水去向
    mdl1.add_constraints(p_gws_sum[h] >= rs_xhl.h_xhl_from[h] + ss_exchanger.h_exch[h] for h in range(0, num_h0))
    ...
    rb = WaterHeatPump(num_h0, mdl1, set_max=20000, set_price=1000, ele_price=ele_price0,
                       case_ratio=np.array([1, 1, 1, 1]))
    rb.cons_register(mdl1)

    sy = WaterHeatPump(num_h0, mdl1, set_max=2000, set_price=3000, ele_price=ele_price0,
                       case_ratio=np.ones(4))
    sy.cons_register(mdl1)
    slj = WaterCooledScrew(num_h0, mdl1, set_max=2000, set_price=1000, ele_price=ele_price0,
                           case_ratio=np.array([1, 0.8]))
    slj.cons_register(mdl1)
    sangk = ThreeGK(num_h0, mdl1, set_max=20000, set_price=1000, ele_price=ele_price0, case_ratio=[1, 0.8, 0.8])
    sangk.cons_register(mdl1)
    doublegk = DoubleGK(num_h0, mdl1, set_max=20000, set_price=1000, ele_price=ele_price0, case_ratio=[1, 0.8])
    doublegk.cons_register(mdl1)
    dire = GeothermalHeatPump(num_h0, mdl1, set_max=20000, set_price=40000, ele_price=ele_price0)
    dire.cons_register(mdl1)
    bx = ESS(num_h0, mdl1, ess_set_max=20000, ess_price=300, pcs_price=1, c_rate_max=0.5, eff=0.9, ess_init=1,
             soc_min=0, soc_max=1)
    bx.cons_register(mdl1)

    xbxl = ESS(num_h0, mdl1, ess_set_max=20000, ess_price=500, pcs_price=1, c_rate_max=0.5, eff=0.9, ess_init=1,
               soc_min=0, soc_max=1)
    xbxl.cons_register(mdl1)

    lowxbxr = ESS(num_h0, mdl1, ess_set_max=20000, ess_price=300, pcs_price=1, c_rate_max=0.5, eff=0.9, ess_init=1,
                  soc_min=0, soc_max=1)
    ...
    mdl1.add_constraints(
        rb.p_sy_cool[h] + p_xcool[h] + sy.p_sy_cool[h] + zq_xhl.c_xhl[h] + rs_xhl.c_xhl[h] + slj.p_slj_cool[h] + p_xice[
            h] + sangk.p_threegk_cool[h] + doublegk.p_doublegk_cool[h] == cool_load[h] for h in range(0, num_h0))
    # p_rb_heat[h]+p_xheat[h]+p_sy_heat[h]+p_gas_heat[h]+p_ss_heat[h]+p_dire[h]+p_sangk_heat[h]==heat_load[h]%热量需求
    mdl1.add_constraints(
        rb.p_sy_heat[h] + p_xheat[h] + sy.p_sy_heat[h] + qs_exchanger.h_exch[h] + ss_exchanger.h_exch[h] +
        sangk.p_threegk_heat[h] + dire.p_dire[h] == heat_load[h] for h in range(0, num_h0))
    # 冰蓄冷逻辑组合
    mdl1.add_constraints(
        sangk.p_threegk_ice[h] + doublegk.p_doublegk_ice[h] + bx.p_ess[h] == p_xice[h] for h in range(0, num_h0))
    lin = Linearization()
    #
    lin.max_zeros(num_h0, mdl1, p_xice, bx.p_ess)
    # 蓄冷逻辑组合
    mdl1.add_constraints(
        rb.p_sy_xcool[h] + sy.p_sy_xcool[h] + slj.p_slj_xcool[h] + sx.p_sx_cool[h] + xbxl.p_ess[h] == p_xcool[h] for h
        in range(0, num_h0))
    lin.max_zeros(num_h0, mdl1, p_xcool, lin.add(num_h0, mdl1, sx.p_sx_cool, xbxl.p_ess))
    # 蓄热逻辑组合
    mdl1.add_constraints(rb.p_sy_xheat[h] + sy.p_sy_xheat[h] + sx.p_sx_heat[h] + lowxbxr.p_ess[h] == p_xheat[h] for h in
                         range(0, num_h0))
    lin.max_zeros(num_h0, mdl1, p_xheat, lin.add(num_h0, mdl1, sx.p_sx_heat, lowxbxr.p_ess))
    # 电量平衡
    ...
    gridnet = GridNet(num_h0, mdl1, gridnet_set_max=200000, set_price=0, ele_price_from=ele_price0, ele_price_to=0.35)
    gridnet.cons_register(mdl1, 2000)
    mdl1.add_constraints(
        dire.ele_dire[h] + slj.ele_slj[h] + rb.ele_sy[h] - bess.p_ess[h] - pv.p_pv[h] + sy.ele_sy[h] + power_load[h] -
        chp.p_chp[h] - diesel.p_diesel[h] + \
        dyzqfsq.p_dyzqfsq[h] + rsdgl.ele_dgl[h] + sangk.ele_threegk[h] + doublegk.ele_doublegk[h] ==
        gridnet.total_power[h] for h in range(0, num_h0))

    iges_set = [diesel, pv, bess, csgr, dyzqfsq, chp, gasgl, qs_exchanger, zq_xhl, pbgr, xbxr, szrs, rsdgl, gasgl_rs,
                sx, shizheng_steam, rs_xhl, ss_exchanger, rb, sy, slj, sangk, doublegk, dire, bx, xbxl, lowxbxr,
                gridnet]
    ...
    pllist.plot_list(
        [dire.ele_dire, slj.ele_slj, rb.ele_sy, bess.p_ess, pv.p_pv, sy.ele_sy, power_load, chp.p_chp,
         diesel.p_diesel, dyzqfsq.p_dyzqfsq, rsdgl.ele_dgl, sangk.ele_threegk, doublegk.ele_doublegk,
         gridnet.total_power],
        ['dire.ele_dire', 'slj.ele_slj', 'rb.ele_sy', 'bess.p_ess', 'pv.p_pv', 'sy.ele_sy', 'power_load', 'chp.p_chp',
         'diesel.p_diesel', 'dyzqfsq.p_dyzqfsq', 'rsdgl.ele_dgl', 'sangk.ele_threegk', 'doublegk.ele_doublegk',
         'gridnet.total_power'], "ele balance")
    plt.figure()
    pllist.plot_list(
        [rb.p_sy_cool, p_xcool, sy.p_sy_cool, zq_xhl.c_xhl, rs_xhl.c_xhl, slj.p_slj_cool, p_xice, sangk.p_threegk_cool,
         doublegk.p_doublegk_cool],
        ['rb.p_sy_cool', 'p_xcool', 'sy.p_sy_cool', 'zq_xhl.c_xhl', 'rs_xhl.c_xhl', 'slj.p_slj_cool', 'p_xice',
         'sangk.p_threegk_cool', 'doublegk.p_doublegk_cool'], 'cool_balance')

    plt.figure()
    pllist.plot_list([rb.p_sy_heat, p_xheat, sy.p_sy_heat, qs_exchanger.h_exch, ss_exchanger.h_exch, sangk.p_threegk_heat,
         dire.p_dire, heat_load],['rb.p_sy_heat', 'p_xheat', 'sy.p_sy_heat', 'qs_exchanger.h_exch', 'ss_exchanger.h_exch', ' sangk.p_threegk_heat', 'dire.p_dire', 'heat_load'],
        'heat_balance')
    plt.figure()
    pllist.plot_list([chp.gts_set.h_exch, chp.yqyrwater_set.h_exch,pbgr.p_pv, xbxr.p_ess,
        szrs.h_citysupply, gasgl_rs.h_gasgl,rsdgl.h_dgl, sx.p_sx_gheat],['chp.gts_set.h_exchh, chp','yqyrwater_set.h_exch','pbgr.p_pv', 'xbxr.p_ess',
        'szrs.h_citysupply', 'gasgl_rs.h_gasgl','rsdgl.h_dgl', 'sx.p_sx_gheat'],'gwheat_balance')
