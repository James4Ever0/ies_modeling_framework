...
with context("基于能路的潮流计算"):
    ...
    for itera in range(MaxIter):
        ...
        # 节点导纳矩阵
        Yg_ = np.matmul(np.matmul(A, yb), A.T) - np.matmul(
            np.matmul(np.matmul(A, yb), ub), Ap.T
        )
        ...
        Gn_1 = node_table[node_table["节点类型"] == "定注入"]["注入 (kg/s)"].values.reshape(
            [-1, 1]
        )  # kg/s
        Gn_1 -= np.matmul(np.matmul(A[fix_G, :], yb), cp.reshape([-1, 1]))
        pn2 = (
            node_table[node_table["节点类型"] == "定压力"]["气压 (MPa)"].values.reshape([-1, 1])
            * 1e6
        )  # Pa
        ...
        I = (
            np.matmul(A.T, p).reshape(-1)
            + cp.reshape(-1)
            - (np.array(Ub).reshape([-1, 1]) * np.matmul(Ap.T, p)).reshape(-1)
        )
        for i in range(numpipe):
            I[i] = abs(
                I[i]
