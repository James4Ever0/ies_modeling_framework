...
with context('数据读取与处理'):
    ...
    tb4 = pd.read_excel('./6节点热网动态data.xls', sheet_name='Dynamic') # what are these sheets?
    # 水力参数
...
with context('动态热力计算'):
    ...
    # 单频网络方程求解
    #这段代码是动态热力计算的主体，根据时域激励分解得到的频域激励，求解管路动态温度响应，最后将频域结果回归到时域。其中，首先根据管路的水力、热力参数计算出稳态时的水流流量、水温，作为时域激励分解得到的频域激励的参考，然后根据管路的热力参数和流量，得到频域下的温度响应，再将其回归到时域，即得到管路在不同时间段内的温度分布情况。
