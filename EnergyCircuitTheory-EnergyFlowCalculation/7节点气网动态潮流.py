...
with context("动态潮流计算"):
    ...
    for itera in range(100):
        ...
        for i in range(numnode + 1):
            TD_E_p2[i, :] = np.interp(
                np.linspace(10, 3600 * 12, 360 * 12),  # 插成10s一个点
        ...
        TD_E_G = np.concatenate(
            (TD_E_G[:, 0].reshape([-1, 1]).repeat(initialization, axis=1), TD_E_G),
            axis=1,
        )
        TD_E_p = np.concatenate(
            (TD_E_p[:, 0].reshape([-1, 1]).repeat(initialization, axis=1), TD_E_p),
            axis=1,
        )
        ...
        for fi in range(FD_E_G.shape[1]):
            ...
            for i in range(numpipe):
                ...
                za = np.cosh(np.sqrt(Ug[i] ** 2 + 4 * Z * Y) / 2 * l[i]) - Ug[
                    i
                ] / np.sqrt(Ug[i] ** 2 + 4 * Z * Y) * np.sinh(
                    np.sqrt(Ug[i] ** 2 + 4 * Z * Y) / 2 * l[i]
                )
                za = za * np.exp(-Ug[i] * l[i] / 2)
                zb = (
                    -2
                    * Z
                    / np.sqrt(Ug[i] ** 2 + 4 * Z * Y)
                    * np.sinh(np.sqrt(Ug[i] ** 2 + 4 * Z * Y) / 2 * l[i])
                )
                ...
                zd = np.cosh(np.sqrt(Ug[i] ** 2 + 4 * Z * Y) / 2 * l[i]) + Ug[
                    i
                ] / np.sqrt(Ug[i] ** 2 + 4 * Z * Y) * np.sinh(
                    np.sqrt(Ug[i] ** 2 + 4 * Z * Y) / 2 * l[i]
                )
            ...
            # 形成支路导纳矩阵
            yb = np.diag(
                np.array([[1 / Zb[i], Yb1[i], Yb2[i]] for i in range(numpipe)]).reshape(
                    -1
                )
            )
            # 形成支路受控电压源矩阵
            ...
            # 支路气压源向量
            cpexp = (
                np.array([[cp[i], 0, 0] for i in range(len(cp))]).reshape([-1, 1])
                if fi == 0
                else np.zeros([18, 1])
            )
            # 形成广义节点导纳矩阵
            Yg_ = np.matmul(np.matmul(A0, yb), A0.T) - np.matmul(
                np.matmul(np.matmul(A0, yb), ub), Ap0.T
            )
            ...
            # 求解网络方程
            FD_E_p[fix_G, fi] = np.matmul(
                np.linalg.inv(Yg_11),
                (
                    (
                        FD_E_G[fix_G, fi]
                        - np.matmul(np.matmul(A0[fix_G, :], yb), cpexp).reshape(-1)
                    )
                    - np.matmul(Yg_12, FD_E_p[fix_p, fi])
                ),
            )
            FD_E_G[fix_p, fi] = (
                np.matmul(Yg_21, FD_E_p[fix_G, fi])
                + np.matmul(Yg_22, FD_E_p[fix_p, fi])
                + np.matmul(np.matmul(A0[fix_p, :], yb), cpexp).reshape(-1)
            )

            # 节点状态变量转支路状态变量
            FD_GL[:, fi] = (
                np.matmul(A.T, FD_E_p[:-1, fi]).reshape(-1)
                - (
                    np.array(Ub).reshape([-1]) * np.matmul(Ap.T, FD_E_p[:-1, fi])
                    - (cp if fi == 0 else np.zeros(numpipe))
                ).reshape(-1)
            ) / np.array(Zb)
            ...
            ) / np.array(Zb) + np.matmul(
                Afrom.T, FD_E_p[:-1, fi].reshape([-1, 1])
            ).reshape(
                -1
            ) * np.array(
                Yb1
        ...
        for fi in range(nf):
            for node in fix_p:
                TD_E_G[node, :] += abs(FD_E_G[node, fi]) * np.cos(
                    2 * np.pi * fi * fr * ts + phase(FD_E_G[node, fi])
                )
        # 定注入节点的压力
        for fi in range(nf):
            for node in fix_G:
                TD_E_p[node, :] += abs(FD_E_p[node, fi]) * np.cos(
                    2 * np.pi * fi * fr * ts + phase(FD_E_p[node, fi])
                )
        # 支路流量
        ...
        for fi in range(nf):
            for branch in range(numpipe):
                TD_GL[branch, :] += abs(FD_GL[branch, fi]) * np.cos(
                    2 * np.pi * fi * fr * ts + phase(FD_GL[branch, fi])
                )
        ...
        print(
            "第%d次迭代，失配误差为%.5f"
            % (itera + 1, np.linalg.norm(v - np.average(abs(TD_v), axis=1)))
        )
