...
class 环境:  # shall be array. not just numbers.
    ...
    """单位：(°C)
    """
    距地面10m处东向风速: List[float]
    """单位：(m/s)
    """
    距地面50m处东向风速: List[float]
    """单位：(m/s)
    """
    距地面10m处北向风谏: List[float]
    """单位：(m/s)
    """
    距地面50m处北向风速: List[float]
    """单位：(m/s)
    """
...
class 燃气轮机_建模仿真(设备):
    def __init__(
        ...
        self.局部压降系数 = self.设备额定运行参数["局部压降系数 "] * 1.00413490930961
...
class 储水罐_建模仿真(设备):
    def __init__(
        ...
        self.入口侧局部压降系数 = self.设备额定运行参数["入口侧局部压降系数"] * 1.00413490930961
        """单位：(megapascal * 年 ** 2 / kilometer ** 6) <- (kilopascal * second ** 2 / meter ** 6) 设备额定运行参数"""
