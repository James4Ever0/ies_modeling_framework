...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 柴油仿真结果(
            元件名称 = safeAbs(model.设备信息.设备名称),
            柴油消耗费用 = safeAbs(((value(model.总成本年化)) * timeParam)),
            柴油消耗量 = safeAbs(((statistics.mean([value(e) for e in model.燃料接口.values()])) * timeParam)),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 光伏发电仿真结果(
            ...
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            产电量 = safeAbs(((statistics.mean([value(e) for e in model.电接口.values()])) * timeParam)),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 柴油发电仿真结果(
            ...
            产电量 = safeAbs(((statistics.mean([value(e) for e in model.电接口.values()])) * timeParam)),
            柴油消耗量 = safeAbs(((statistics.mean([value(e) for e in model.燃料接口.values()])) * timeParam)),
            平均效率_平均COP = safeAbs(safeDiv(((statistics.mean([value(e) for e in model.电接口.values()])) * timeParam),model.燃料热值 * (((statistics.mean([value(e) for e in model.燃料接口.values()])) * timeParam)))),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 锂电池仿真结果(
            ...
            设备台数 = safeAbs(value(model.DeviceCount)),
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            平均效率_平均COP = safeAbs(safeDiv(ReLU((((statistics.mean([ReLU(e) for e in model.电接口.values()])) * timeParam)) - (model.InitSOC * model.TotalCapacity)) , (-(((statistics.mean([-ReLU(-e) for e in model.电接口.values()])) * timeParam))))),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 变压器仿真结果(
            ...
            设备台数 = safeAbs(value(model.DeviceCount)),
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            平均效率_平均COP = safeAbs(- safeDiv(statistics.mean([value(e) for e in model.电输入.values()]),statistics.mean([value(e) for e in model.电输出.values()]))),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 双向变流器仿真结果(
            ...
            设备台数 = safeAbs(value(model.DeviceCount)),
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            平均效率_平均COP = safeAbs(value((safeDiv(sumVarList(model.储能端_.x_pos),sumVarList(model.线路端_.x_neg)) * sumVarList(model.储能端_.b_pos))+ (safeDiv(sumVarList(model.线路端_.x_pos),sumVarList(model.储能端_.x_neg)) * sumVarList(model.线路端_.b_pos)))/model.计算参数.迭代步数),
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 锂电池出力曲线(
            ...
            荷电状态 = [value(safeDiv(e , model.TotalCapacity)) for e in model.CurrentTotalCapacity.values()],
        )
...
[% do assert(paramKeys.__eq__(objKeys), "devName: {}\n{} (code) != {} (template)".format(devName, paramKeys, objKeys)) %]
        return 双向变流器出力曲线(
            时间 = list(range(model.计算参数.迭代步数)),
            元件名称 = model.设备信息.设备名称,
            转换功率 = addListElem([value(e) for e in model.储能端_.x_pos.values()], [value(e) for e in model.线路端_.x_pos.values()]),
        )
