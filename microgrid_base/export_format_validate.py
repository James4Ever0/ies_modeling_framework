...
class 光伏发电仿真结果(BaseModel):
    ...
    def export(model: 光伏发电模型, timeParam:float):
        ...
        return 光伏发电仿真结果(
            ...
            产电量 = safeAbs(((statistics.mean([value(e) IV(vlist)])) * timeParam)),
        )
...
class 柴油发电仿真结果(BaseModel):
    ...
    def export(model: 柴油发电模型, timeParam:float):
        ...
        return 柴油发电仿真结果(
            ...
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            产电量 = safeAbs(((statistics.mean([value(e) IV(vlist)])) * timeParam)),
            柴油消耗量 = safeAbs(((statistics.mean([value(e) IV(vlist)])) * timeParam)),
            平均效率_平均COP = safeAbs(safeDiv(((statistics.mean([value(e) IV(vlist)])) * timeParam),model.燃料热值 * (((statistics.mean([value(e) IV(vlist)])) * timeParam)))),
        )
class 锂电池仿真结果(BaseModel):
    ...
    def export(model: 锂电池模型, timeParam:float):
        ...
        return 锂电池仿真结果(
            ...
            设备台数 = safeAbs(value(model.DeviceCount)),
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            平均效率_平均COP = safeAbs(safeDiv(ReLU((((statistics.mean([ReLU(e) IV(vlist)])) * timeParam)) - (model.InitSOC * model.TotalCapacity)) , (-(((statistics.mean([-ReLU(-e) IV(vlist)])) * timeParam))))),
        )
class 变压器仿真结果(BaseModel):
    ...
    def export(model: 变压器模型, timeParam:float):
        ...
        return 变压器仿真结果(
            ...
            设备台数 = safeAbs(value(model.DeviceCount)),
            设备维护费用 = safeAbs(((value(model.总固定维护成本 + model.总可变维护成本年化)) * (timeParam/8760))),
            平均效率_平均COP = safeAbs(- safeDiv(statistics.mean([value(e) IV(vlist)]),statistics.mean([value(e) IV(vlist)]))),
        )
...
class 锂电池出力曲线(BaseModel):
    ...
    def export(model: 锂电池模型, timeParam:float):
        ...
        return 锂电池出力曲线(
            时间 = list(range(model.计算参数.迭代步数)),
            元件名称 = model.设备信息.设备名称,
            电功率 = [value(e) IV(vlist)],
            荷电容量 = [value(e) IV(vlist)],
            荷电状态 = [value(safeDiv(e , model.TotalCapacity)) IV(vlist)],
        )
...
class 双向变流器出力曲线(BaseModel):
    ...
    def export(model: 双向变流器模型, timeParam:float):
        ...
        return 双向变流器出力曲线(
            时间 = list(range(model.计算参数.迭代步数)),
            元件名称 = model.设备信息.设备名称,
            转换功率 = addListElem([value(e) IV(vlist)], [value(e) IV(vlist)]),
        )
