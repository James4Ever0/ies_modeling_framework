...
coax_triplets = [ # Input, Output, ConnectionBaseName
    (Output("供电端"), Input("变流器"), "供电端母线", False),
    (IO("储能端"), IO("双向变流器"), "储能端母线", True),
    (Input("母线"), Output("母线"), "母线", False),
    (Input("柴油"), Output('柴油'), "柴油母线", False)
]
...
for i,o , wire_name, is_io in coax_triplets:
    if is_io:
        start = IO(o)
        end = IO(o)
    else:
