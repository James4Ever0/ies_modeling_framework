...
class 风力发电信息(设备信息):
    ...
    normalizedPower:Union[None, List[float]]= Field(title="风力发电标幺值", description = "空或数组(典型日长度为24,全年逐时长度为8760,秒级长度为7200)")
...
class 锂电池模型(设备模型):
    ...
    def constraints_register(self):
        ...
        if self.needStorageDecayCompensation:
            ...
            self.CustomRangeConstraint( self.原电接口.x, self.CurrentTotalCapacity, customRange = range(self.计算参数.迭代步数-1),expression =  lambda x,y,i: x[i] == (y[i] - y[i+1]) * self.计算参数.时间参数)
        ...
        if self.计算参数.计算类型 == '设计规划':
            if self.设备信息.循环边界条件 == '日间独立':
                ...
                if self.needStorageDecayCompensation:
                    ...
                    self.mw.Constraint(self.原电接口.x[0] == (self.CurrentTotalCapacity[self.计算参数.迭代步数-1] - self.CurrentTotalCapacity[0]) * self.计算参数.时间参数 )
            else:
        ...
        self.总可变维护成本年化 = ((计算范围内总平均功率*self.计算参数.迭代步数) / self.计算参数.迭代步数) * 每年小时数 * self.VariationalCostPerWork 
        # avg_power * 8760 = annual_work
