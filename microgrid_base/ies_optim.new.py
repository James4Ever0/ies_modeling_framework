...
class 柴油发电信息(设备信息):
    ...
    maximumAnnualOperatingTimeLimitOfTheUnit:Union[conint(ge=0), None]=Field(default =None, title="机组年运行时间最高限值", description="单位：次")
    considerUnitStartUpCosts:bool=Field(default = False, title="考虑机组启动费用", description="若选否，变量约束不创建，变量为自由变量，降低计算量")
    unitSingleStartupCost:Union[conint(ge=0), None]=Field(default =None, title="机组单次启动费用", description="单位：元/次")
    unitPlanningAlgorithmSelection: 油耗规划算法=Field(default =油耗规划算法.平均 ,title="机组规划算法选择", description="平均/最佳")
    averageLoadRate :Union[confloat(ge=0, le=100), None] = Field(default =None, title="平均负载率", description="如果是平均油耗规划算法，需要填写，否则默认为空")
...
class ModelWrapper:
    ...
    obj_expr = property(fget = obj_exprGetter, fset = obj_exprSetter)
        ...
        self.submodelClassNameToVarName: Dict[str, List[str]] = {self._submodelClassName: []}
    ...
    def setSubmodelClassName(self, submodelClassName:str):
        ...
        self.submodelClassNameToVarName = self.submodelClassNameToVarName.get(submodelClassName, [])
...
class 变压器模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.RangeConstraint(self.电输入, self.电输出, lambda x,y: x * self.Efficiency * self.PowerParameter == -y)
...
class 电负荷模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.IncomeRates = [self.PriceModel.getFee(power, getTimeInDay(index)) for index, power in enumerate(self.电接口.values())] # positive?
...
class 规划结果详情(BaseModel):
    ...
    def export(cls, deviceModel: 设备模型协议, deviceSimulationResult, timeParam:float):
        ...
        params['平均效率_平均COP'] = getattr(deviceSimulationResult, 'averageEfficiency', cmath.nan)
        ...
        params['设备年维护费'] = getattr(deviceSimulationResult, "设备维护费用", cmath.nan) # 乘过时间参数就不用乘了
...
class 设备节点(节点基类):
    ...
    param:Union[柴油信息,电负荷信息,光伏发电信息,风力发电信息,柴油发电信息,锂电池信息,变压器信息,变流器信息,双向变流器信息,传输线信息] = Field(title = '设备信息', description = "不同设备有不同的信息格式")
