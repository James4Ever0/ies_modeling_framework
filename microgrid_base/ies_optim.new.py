...
class 柴油信息(设备基础信息):
    def toStandard(self, attr:str):
        ...
        unit = desc.replace("：", ":").split(":")[-1].strip()
        assert unit, f"{className}.{attrName} does not have unit definition"
...
class 光伏发电模型(设备模型):
    def __init__(self, PD:dict, mw: ModelWrapper, 计算参数实例: 计算参数, 设备ID: 光伏发电ID, 设备信息: 光伏发电信息):
        ...
        self.机组年启动次数 = ...
        self.机组年运行时间 = ...
unt = math.ceil(self.MinInstallArea/self.Area)
            assert ount>=self        self.电功率中转 = self.变量列表_带指示变量("电功率中转")
...
class 规划结果详情(BaseModel):
    ...
    def export(cls, deviceModel: 设备模型协议, deviceSimulationResult, timeParam:float):
        ...
        par平均效率_平均COPtr(deviceModel.设备信息, '设备型号', '')
