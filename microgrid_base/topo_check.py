...
# 设备、母线、连接线、合并线
锚点类型 = ['双向变流器线路端输入输出', '变流器输入', '供电端输出', '双向变流器储能端输入输出', '负荷电输入', '变压器输出', '电储能端输入输出', '电母线输出', '电母线输入', '柴油输出', '柴油输入']

母线类型 = ['可连接电储能端母线', '可连接供电端母线', '可连接电母线', '可连接负荷电母线', '可连接柴油母线']

连接线类型 = ['不可连接电母线输出', '不可连接电储能端母线输出', '不可连接电母线输入', '不可连接供电端母线输入输出', '不可连接负荷电母线输入', '不可连接负荷电母线', '不可连接供电端母线输出', '不可连接电储能端母线输入', '不可连接电储能端母线', '不可连接负荷电母线输入输出', '不可连接电储能端母线输入输出', '不可连接供电端母线', '不可连接电母线输入输出', '不可连接负荷电母线输出', '不可连接电母线', '不可连接供电端母线输入', '不可连接柴油母线', '不可连接柴油母线输入输出', '不可连接柴油母线输入', '不可连接柴油母线输出']

合并线类型 = ['可合并电储能端母线', '可合并负荷电母线', '可合并供电端母线', '可合并电母线', '可合并柴油母线']

设备类型 = ['柴油', '电负荷', '光伏发电', '风力发电', '柴油发电', '锂电池', '变压器', '变流器', '双向变流器', '传输线']


设备接口集合 = {'柴油': {('燃料接口', '柴油输出')}, '电负荷': {('电接口', '负荷电输入')}, '光伏发电': {('电接口', '供电端输出')}, '风力发电': {('电接口', '供电端输出')}, '柴油发电': {('电接口', '供电端输出'), ('燃料接口', '柴油输入')}, '锂电池': {('电接口', '电储能端输入输出')}, '变压器': {('电输入', '电母线输入'), ('电输出', '变压器输出')}, '变流器': {('电输入', '变流器输入'), ('电输出', '电母线输出')}, '双向变流器': {('线路端', '双向变流器线路端输入输出'), ('储能端', '双向变流器储能端输入输出')}, '传输线': {('电输入', '电母线输入'), ('电输出', '电母线输出')}}
连接类型映射表 = {frozenset({'可连接电母线', '双向变流器线路端输入输出'}): '不可连接电母线输入输出', frozenset({'变流器输入', '供电端输出'}): '不可连接供电端母线', frozenset({'可连接供电端母线'}): '可合并供电端母线', frozenset({'可连接供电端母线', '变流器输入'}): '不可连接供电端母线输出', frozenset({'可连接供电端母线', '供电端输出'}): '不可连接供电端母线输入', frozenset({'变压器输出', '负荷电输入'}): '不可连接负荷电母线', frozenset({'可连接负荷电母线'}): '可合并负荷电母线', frozenset({'可连接负荷电母线', '负荷电输入'}): '不可连接负荷电母线输出', frozenset({'可连接负荷电母线', '变压器输出'}): '不可连接负荷电母线输入', frozenset({'柴油输入', '柴油输出'}): '不可连接柴油母线', frozenset({'可连接柴油母线'}): '可合并柴油母线', frozenset({'柴油输入', '可连接柴油母线'}): '不可连接柴油母线输出', frozenset({'柴油输出', '可连接柴油母线'}): '不可连接柴油母线输入', frozenset({'电母线输出', '电母线输入'}): '不可连接电母线', frozenset({'可连接电母线'}): '可合并电母线', frozenset({'可连接电母线', '电母线输入'}): '不可连接电母线输出', frozenset({'可连接电母线', '电母线输出'}): '不可连接电母线输入', frozenset({'电储能端输入输出', '双向变流器储能端输入输出'}): '不可连接电储能端母线', frozenset({'可连接电储能端母线'}): '可合并电储能端母线', frozenset({'电储能端输入输出', '可连接电储能端母线'}): '不可连接电储能端母线输出', frozenset({'可连接电储能端母线', '双向变流器储能端输入输出'}): '不可连接电储能端母线输入'}
            ...
            if node_type == "母线":
                ...
                for n in neighbors:
                    ...
                    if ne_type == "合并线":
                        ...
                        assert ne_subtype.replace("合并", "连接") == node_subtype, f"节点 #node_id"
                    elif ne_type == "连接线":
                        assert ne_subtype in 连接线类型, f"节点 #node_id"
                        assert ne_subtype.replace('不可','可').replace("输入","").replace("输出","") == node_subtype, f"节点 #node_id"
                    else:
                ...
                for n in neighbors:
                    ...
                    assert len(list(self.G.neighbors(n))) == 2, f"节点 #n 相邻节点数错误: {len(list(self.G.neighbors(n)))} 相邻节点: {(list(self.G.neighbors(n)))}"
                    port_set.add((port_name, ne_subtype))

                assert port_set == 设备接口集合[node_subtype], f"节点 #node_id  PORT SET: {port_set} TARGET: {设备接口集合[node_subtype]}"
            elif node_type == "连接线":
                assert node_subtype in 连接线类型, f'节点 #node_id 不合理连接线类型： {node_subtype}'
                assert len(neighbors) == 2, f'节点 #node_id 不合理连接线相邻节点数: {len(neighbors)} 相邻节点: {neighbors}'
                dev_ids = set()
                subtypes = []
                ...
                for n in neighbors:
                    ...
                    if ne_type == "锚点":
                        ...
                        dev_ids.add(n)
                assert len(dev_ids) == 2, f'节点 #node_id invalid dev_ids: {dev_ids}' # no self-connection.
                assert 连接类型映射表[frozenset(subtypes)] == node_subtype,  f"节点 #node_id 未知连接组合: (两端: {subtypes} 连接线: {node_subtype})"
            elif node_type == "合并线":
                合并线ID列表.append(node_id)
                assert node_subtype in 合并线类型, f"节点 #node_id 未知合并线类型: {node_subtype}"
                assert len(neighbors) == 2, f"节点 #node_id 不合理相邻节点数: {len(neighbors)} 相邻节点: {len(neighbors)}"
                node_ids = set()
                ...
                for n in neighbors:
                    ...
                    assert ne_type == "母线", f"节点 #n 不合理类型： 
                    {ne_type}"
                assert len(node_ids) == 2, f"节点 #node_id 不合理合并线总节点数：{len(node_ids)} 节点列表: {node_ids}"
            elif node_type == '锚点':
                continue
            else:
        ...
        for id_set in self.合并母线ID集合列表:
            ...
            if has_input and has_output:
                ...
                if output_conns[0] == "不可连接柴油母线输出":
                    assert len(output_conns) == 1, "柴油母线有且只有一个与柴油元件的连接"
            else:
                print()
                print("============ERROR LOG============")
                print()
...
class 变流器(设备):
    def __init__(self, topo:拓扑图, **kwargs):
        super().__init__(topo=topo, device_type="变流器",  port_definition={('电输入', '变流器输入'), ('电输出', '电母线输出')}, **kwargs)
...
class 双向变流器(设备):
    def __init__(self, topo:拓扑图, **kwargs):
        super().__init__(topo=topo, device_type="双向变流器",  port_definition={('线路端', '双向变流器线路端输入输出'), ('储能端', '双向变流器储能端输入输出')}, **kwargs)
...
class 传输线(设备):
    def __init__(self, topo:拓扑图, **kwargs):
        super().__init__(topo=topo, device_type="传输线",  port_definition={('电输入', '电母线输入'), ('电输出', '电母线输出')}, **kwargs)
