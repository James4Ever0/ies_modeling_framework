...
class PiecewiseInfo(BaseModel):
    ...
    # autogenerated.
    output_violation: float = 0
    input_domain_violation: float = 0
    input_domain: Tuple[float, float] = (0, 0)

    def calculate_input_domain_and_violation(self, tol=1e-6):
        self.input_domain = (min(self.input_points), max(self.input_points))

        input_point = self.inputVarInfo.val
        output_point = self.outputVarInfo.val

        self.input_domain_violation = get_bounds_violation(
            input_point, *self.input_domain, tol=0
        )
        if self.input_domain_violation == 0:  # within bound.
            expected_output = np.interp(
                input_point, self.input_points, self.output_points
            )
            output_violation = abs(output_point - expected_output)
            self.output_violation = moderate_violation(output_violation, tol)
    ...
    def piecewise(self):
        for pw in self.model.block_data_objects(active=True, descend_into=True):
            if isinstance(pw, PiecewiseType):
                ...
                piecewiseInfo.calculate_input_domain_and_violation(tol=self.tol)
