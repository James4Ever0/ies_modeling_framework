...
class SoilSource(IGES):
    ...
    def __init__(self, num_h, mdl, cool_max, cool_min, heat_max, heat_min, set_price, set_name='SoilSource'):
        self.num_h = num_h
        IGES(set_name)
        SoilSource.index += 1
        ...
        self.pcool_out = self.mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                      name='SoilSource.pcool_out{0}'.format(SoilSource.index),
                                                      lb=-1e3)
        self.pheat_out = self.mdl.continuous_var_list([i for i in range(0, self.num_h)],
                                                      name='SoilSource.tpheat{0}'.format(SoilSource.index),
                                                      lb=-1e3)
        self.nianhua = self.mdl.continuous_var(name='SoilSource_nianhua{0}'.format(SoilSource.index))
        self.length = self.mdl.continuous_var(name='SoilSourcelength{0}'.format(SoilSource.index))
        self.nset = self.mdl.continuous_var(name='SoilSource.nset{0}'.format(SoilSource.index))
        self.zcool = self.mdl.integer_var_list([i for i in range(self.num_h)],
                                               name='SoilSource.zcool{0}'.format(SoilSource.index))
        self.zheat = self.mdl.integer_var_list([i for i in range(self.num_h)],
        ...
        self.Rb = 0.5 * (1 / (2 * self.pi * self.lamdab) * (
                np.log(self.db / self.do) + np.log(self.db / self.D) + (self.lamdab - self.lamdas) / (
                self.lamdab + self.lamdas) * np.log(
            math.pow(self.db, 4) / (math.pow(self.db, 4) - math.pow(self.D, 4)))) +
                         1 / (2 * self.pi * self.lamdap) * np.log(self.do / self.di) + 1 / (
                                 self.pi * self.di * self.lh))
    ...
    def cons_register(self, mdl,loop_flag):
        ...
        mdl.add_constraints(self.pcool_out[h] <= 4 * self.nset for h in range(self.num_h))
        mdl.add_constraints(self.pcool_out[h] >= 0 for h in range(self.num_h))
        mdl.add_constraints(self.pheat_out[h] <= 4 * self.nset for h in range(self.num_h))
        mdl.add_constraints(self.pheat_out[h] >= 0 for h in range(self.num_h))
        mdl.add_constraints(self.pheat_out[h] <= self.zheat[h] * bigM for h in range(self.num_h))
        mdl.add_constraints(self.pcool_out[h] <= self.zcool[h] * bigM for h in range(self.num_h))
        mdl.add_constraints(self.zcool[h] + self.zheat[h] <= 1 for h in range(self.num_h))
    ...
    def Do_simulation(self, sol_run, periodh,consideer_history):
        ...
        self.q = [1000 * (pcool_out[index] - pheat_out[index]) / length for index in range(self.num_h)]
        print('do simulation')

        num_period = round(8760 / periodh)
        for period in range(num_period):
            self.qav[period] = sum(self.q[period * periodh:(period + 1) * periodh]) / periodh
        ...
        for period in range(num_period):
            ...
            self.qln_sumf[period] = sum(
                self.qav[period + 1 - nindex] * np.log((nindex + 1) / (nindex)) for nindex in range(1, period + 1))

            self.qln_sumb[period] = sum(
                self.qav[period + 1 - nindex + num_period - 1] * np.log((nindex + 1) / (nindex)) for nindex in
                range(period + 1, num_period))
            if consideer_history==1:
                self.Tfav[period] = self.k * (self.qln_sumf[period]+self.qln_sumb[period] ) + self.qav[
                    period] * self.Rb + self.Tff
                self.Tfav_1[period] = self.k * (self.qln_sumf[period]++self.qln_sumb[period])
            else:
                self.Tfav[period] = self.k * (self.qln_sumf[period]) + self.qav[
                    period] * self.Rb + self.Tff
