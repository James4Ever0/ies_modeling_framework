...
class 光伏发电模型(设备模型):
    ...
    def constraints_register(self):
        ...
        总可变维护成本年化 = (self.SumRange(self.电输出)) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork # avg_power * 8760 = annual_work
...
class 锂电池模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 锂电池ID, 设备信息: 锂电池信息):
        ...
        self.ports['电接口'] = self.电接口 = self.变量列表("电接口", within=Reals)
        """
        类型: 电储能端输入输出
        """
    ...
    def constraints_register(self):
        ...
        总可变维护成本年化 = (计算范围内总电变化量) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork # avg_power * 8760 = annual_work
...
class 变压器模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 变压器ID, 设备信息: 变压器信息):
        ...
        if self.计算参数.计算类型 == "设计规划": # 在变压器和负荷的交换节点处做处理
            self.最大允许的负载总功率 = self.DeviceCount*(self.RatedPower*self.Efficiency)*self.PowerParameter/self.LoadRedundancyParameter
    ...
    def constraints_register(self):
        ...
        总可变维护成本年化 = (-self.SumRange(self.电输入)) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork # avg_power * 8760 = annual_work
...
class 双向变流器模型(设备模型):
    ...
    def constraints_register(self):
        ...
        总可变维护成本年化 = ((self.SumRange(self.储能端_.x_neg)+self.SumRange(self.线路端_.x_neg))) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork # avg_power * 8760 = annual_work
...
class 电负荷(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 电负荷ID, 设备信息: 电负荷信息):
        ...
        self.ports['电接口'] = self.电接口 = self.变量列表("电接口", within=NegativeReals)
        """
        类型: 负荷电输入
        """
