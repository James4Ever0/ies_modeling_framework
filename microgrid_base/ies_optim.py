...
class 设备模型:
    ...
    def SumRange(self,var_1):
        return reduce(sequence = [var_1[i] for i in range(self.计算参数.迭代步数)], function = lambda x,y: x+y)
...
class 光伏发电模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 光伏发电ID, 设备信息: 光伏发电信息):
...
class 风力发电模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 风力发电ID, 设备信息: 风力发电信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 柴油发电模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 柴油发电ID, 设备信息: 柴油发电信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
    ...
    def constraints_register(self):
        ...
        self.RangeConstraintMulti(self.电功率中转.x_pos, self.电输出, self.电功率中转.b_pos, expression = lambda x,y,z: x + 总最小启动功率*z == y)
...
class 锂电池模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 锂电池ID, 设备信息: 锂电池信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
    ...
    def constraints_register(self):
        ...
        self.CustomRangeConstraint( self.原电接口.x, self.CurrentTotalActualCapacity, range(self.计算参数.迭代步数-1), lambda x,y,i: x == (y[i] - y[i+1]) * self.计算参数.时间参数)
        ...
        for i in range(self.计算参数.迭代步数-1):
            Constraint(self.CurrentTotalActualCapacity[i+1] - self.CurrentTotalActualCapacity[i] < self.MaxTotalCapacityDeltaPerStep)
            Constraint(self.CurrentTotalActualCapacity[i+1] - self.CurrentTotalActualCapacity[i] > -self.MaxTotalCapacityDeltaPerStep)
        ...
        if self.计算参数.计算类型 == "设计规划":
            if self.设备信息.循环边界条件 == '日间独立':
                ...
                Constraint(self.CurrentTotalActualCapacity[0] - self.CurrentTotalActualCapacity[self.计算参数.迭代步数-1] < self.MaxTotalCapacityDeltaPerStep)

                Constraint(self.CurrentTotalActualCapacity[0] - self.CurrentTotalActualCapacity[self.计算参数.迭代步数-1] > -self.MaxTotalCapacityDeltaPerStep)

                Constraint(self.原电接口.x[0] == (self.CurrentTotalActualCapacity[self.计算参数.迭代步数-1] - self.CurrentTotalActualCapacity[0]) * self.计算参数.时间参数 )
            else:
...
class 变压器模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 变压器ID, 设备信息: 变压器信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 变流器模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 变流器ID, 设备信息: 变流器信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 双向变流器模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 双向变流器ID, 设备信息: 双向变流器信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 传输线模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 传输线ID, 设备信息: 传输线信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 电负荷模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 电负荷ID, 设备信息: 电负荷信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
class 柴油模型(设备模型):
    def __init__(self, PD:dict, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 柴油ID, 设备信息: 柴油信息):
        super().__init__(PD=PD, model=model, 计算参数实例=计算参数实例, ID = 设备ID.ID)
        self.设备ID = 设备ID
        self.设备信息 = 设备信息
...
def compute(devs:List[dict], adders:Dict[int,dict], graph_data:dict, G: Graph, model: ConcreteModel):
    PD = {}
    ...
    for adder_index, adder in adders.items():
        ...
        for j in range(algoParam.迭代步数):
            seqsum = reduce(sequence=[PD[i][j] for i in input_indexs + output_indexs + io_indexs], function = lambda x,y: x+y)
