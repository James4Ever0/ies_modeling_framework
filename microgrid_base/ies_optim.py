...
class 分月电价(BaseModel, 电价转换):
    PriceList:Tuple[float, float, float, float, float, float, float, float, float, float, float, float] =Field(title = "长度为12的价格数组", description = "单位: 元/kWh")
...
class 分时电价(BaseModel, 电价转换):
    PriceList: Tuple[float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float, float] = Field(title = "长度为24的价格数组", description = "单位: 元/kWh")
...
class 分时分月电价(BaseModel, 电价转换):
    PriceStruct:Tuple[分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价, 分时电价] = Field(title = "长度为12的分时电价数组", description = "单位: 元/kWh")
    def getFee(self, power:float, time_in_day:float) -> float:
...
class 分时阶梯电价(BaseModel):
    PriceStructList:Tuple[阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价, 阶梯电价] = Field(title = "长度为24的阶梯电价列表", description = "单位: 元/kWh")
...
class 锂电池信息(设备信息):
    ...
    TotalDischargeCapacity: confloat(ge=0) = Field(title = "生命周期总放电量", description =   "名称: 生命周期总放电量\n单位: kWh")
    """
    名称: 生命周期总放电量
    单位: kWh
    """
...
class ModelWrapper:
    ...
    def Constraint(self,  *args, **kwargs):
        ...
        if "initialize" in kwargs.keys(): del kwargs["initialize"]
...
class 光伏发电模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.总可变维护成本年化 = ((self.SumRange(self.电输出)) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork 
...
class 锂电池模型(设备模型):
    def __init__(self, PD:dict, mw: ModelWrapper, 计算参数实例: 计算参数, 设备ID: 锂电池ID, 设备信息: 锂电池信息):
        ...
        if self.needStorageDecayCompensation: 
            ...
            if self.计算参数.计算类型 == "设计规划":
                ...
                self.ActualTotalDecayRateCompensated = self.变量列表('总补偿衰减率', bounds = (0,(self.BatteryStorageDecay/100)*self.MaxTotalCapacity), within=NonNegativeReals) # the greater the value, the less our compensation is, the greater the real discharge by decay is (will not emit to external ports).
                # constraint.
                self.RangeConstraintMulti(self.ActualTotalDecayRateCompensated, expression=lambda x: x<=self.TotalStorageDecayRate)
            else:
                self.ActualTotalDecayRateCompensated = self.变量列表('总补偿衰减率', bounds = (0,self.TotalStorageDecayRate), within=NonNegativeReals)
    ...
    def constraints_register(self):
        ...
        if self.needStorageDecayCompensation:
            ...
            self.CustomRangeConstraint( self.原电接口.x, self.CurrentTotalActualCapacity, customRange = range(self.计算参数.迭代步数-1),expression =  lambda x,y,i: x[i] == (y[i] - y[i+1]) * self.计算参数.时间参数)
        ...
        self.总可变维护成本年化 = ((计算范围内总平均功率*self.计算参数.迭代步数) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork 
        # avg_power * 8760 = annual_work
...
class 变压器模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.总可变维护成本年化 = ((-self.SumRange(self.电输入)) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork 
        # avg_power * 8760 = annual_work
...
class 双向变流器模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.总可变维护成本年化 = (((self.SumRange(self.储能端_.x_neg)+self.SumRange(self.线路端_.x_neg))) / self.计算参数.迭代步数) * 8760 * self.VariationalCostPerWork 
        # avg_power * 8760 = annual_work
