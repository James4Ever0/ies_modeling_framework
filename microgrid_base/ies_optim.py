...
class 光伏发电模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 光伏发电ID, 设备信息: 光伏发电信息):
        ...
        if self.计算参数.计算类型 == "设计规划":        self.MaxInstallArea: float = 设备信息.MaxInstallArea
            """
            名称: 最大安装面积
            单位: m2
            """
        ...
        if self.计算参数.计算类型 == "设计规划":        self.MinInstallArea: float = 设备信息.MinInstallArea
            """
            名称: 最小安装面积
            单位: m2
            """
    ...
    总可变维护成本年化 = (self.SumRange(self.电输出)) * (8760 / self.计算参数.迭代步数) * ((1 if self.计算参数.计算步长 == "小时" else 3600)) *self.VariationalCostPerPower
...
class 风力发电模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 风力发电ID, 设备信息: 风力发电信息):
        ...
        if self.计算参数.计算类型 == "设计规划":        self.MaxDeviceCount: float = 设备信息.MaxDeviceCount
            """
            名称: 最大安装台数
            单位: 台
            """
        ...
        if self.计算参数.计算类型 == "设计规划":        self.MinDeviceCount: float = 设备信息.MinDeviceCount
            """
            名称: 最小安装台数
            单位: 台
            """
        ...
        if self.计算参数.计算类型 == "仿真模拟":        self.DeviceCount: float = 设备信息.DeviceCount
            """
            名称: 安装台数
            单位: 台
            """
    ...
    总建设费用 = self.BuildCostPerWatt * (self.DeviceCount * self.RatedPower) + self.BuildBaseCost
...
class 锂电池模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 锂电池ID, 设备信息: 锂电池信息):
        ...
        if self.计算参数.计算类型 == "设计规划":        self.InitSOC: float = 设备信息.InitSOC * 0.01
            """
            名称: 初始SOC
            单位: one <- percent
            """
        ...
        self.MaxTotalCapacityDelta = self.BatteryDeltaLimit/((1 if self.计算参数.计算步长 == "小时" else 3600)) * self.TotalCapacity

        self.TotalStorageDecay = self.BatteryStorageDecay /100/((1 if self.计算参数.计算步长 == "小时" else 3600)) * self.TotalCapacity
    ...
    总建设费用 = self.BuildCostPerWatt * (self.DeviceCount * self.RatedCapacity) + self.BuildBaseCost

    总固定成本年化 = (总采购成本 + 总固定维护成本 + 总建设费用) * 年化率



    总可变维护成本年化 = (计算范围总电变化量) * (8760 / self.计算参数.迭代步数) * ((1 if self.计算参数.计算步长 == "小时" else 3600)) *self.VariationalCostPerPower
...
class 变压器模型(设备模型):
    ...
    总可变维护成本年化 = (-self.SumRange(self.电输入)) * (8760 / self.计算参数.迭代步数) * ((1 if self.计算参数.计算步长 == "小时" else 3600)) *self.VariationalCostPerPower
...
class 双向变流器模型(设备模型):
    ...
    def constraints_register(self):
        ...
        self.RangedConstraint(self.线路端_.x_neg, self.储能端_.x_pos,lambda x,y: x = y*self.Efficiency)
    ...
    总可变维护成本年化 = ((self.SumRange(self.储能端_.x_neg)+self.SumRange(self.线路端_.x_neg))) * (8760 / self.计算参数.迭代步数) * ((1 if self.计算参数.计算步长 == "小时" else 3600)) *self.VariationalCostPerPower
...
class 传输线模型(设备模型):
    def __init__(self, model: ConcreteModel, 计算参数实例: 计算参数, 设备ID: 传输线ID, 设备信息: 传输线信息):
        ...
        if self.计算参数.计算类型 == "设计规划":        self.Length: float = 设备信息.Length
            """
            名称: 长度
            单位: kilometer
            """
        ...
        if self.计算参数.计算类型 == "仿真模拟":        self.Length: float = 设备信息.Length
            """
            名称: 长度
            单位: kilometer
            """
    ...
    def constraints_register(self):
        ...
        # 输出输入功率约束
        TotalDecayPerStep = self.Length * self.PowerTransferDecay / (1 if self.计算参数.计算步长 == "小时" else 3600)
